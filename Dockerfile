...
RUN git clone https://github.com/tree-sitter/tree-sitter-ruby.git && \
    cd tree-sitter-ruby && \
    gcc -o ruby.so -I./src src/parser.c src/scanner.c -shared -Os -fPIC && \
    mkdir -p /usr/lib/nvim/parser && \
    mv ruby.so /usr/lib/nvim/parser/ && \
    cd .. && \
